module WaveCycleStochasticDriftModel
...
function state_space(θ, cycle_order)
    ...
    # Observation equation: yₜ = uₜ + ψ_{max,t} + εₜ.
    Z = zeros(1, state_dim)
    Z[1, 1] = 1       # trend state uₜ
    Z[1, 3] = 1       # cosine part of highest order cycle (ψ)
    ...
    # Selection matrix: trend innovation and cycle innovation for the lowest block.
    R = zeros(state_dim, 3)
    R[2, 1] = 1   # trend slope gets ξₜ
    idx_low = 2 + 2*(cycle_order - 1) + 1
    R[idx_low,     2] = 1   # cycle state gets κₜ
    R[idx_low+1,   3] = 1   # complementary cycle state gets κₜ
