...
#########################
function positive_definite_check(model, θ, obs_dim)
    if obs_dim == 1
        return true
    else
        Z, H, d, T, R, Q, c, P_diffuse = get_state_space(model, θ)
...
# Negative Likelihood 
########################
function neg_log_likelihood(θ, model, y, X, a1, P1; filter_type="kalman", N_particles=1000, rng=Random.GLOBAL_RNG)
    if filter_type == "kalman"
        LogL, _, _ = diffuse_kalman_filter_alternative(model, y, X, θ, a1, P1; do_smooth = false, do_sim_smooth = false, rng=rng)
        return -LogL
    elseif filter_type == "particle"
        logL, _ = ParticleFilter.particle_filter(model, y, θ, a1, P1; N_particles=N_particles)
        return -logL
    else
...
# Log Posterior
#########################
function log_posterior(model, θ,  log_jac, prior_info, y, X, a1, P1; filter_type="kalman", N_particles=1000, rng=Random.GLOBAL_RNG)
    log_lik = -neg_log_likelihood(θ, model, y, X, a1, P1; filter_type=filter_type, N_particles=N_particles, rng=rng)
...
#########################
function MCMC_estimation(model,y, X, prior_info_collection, a1, P1;
              filter_type="kalman",
              iter_init = 20000,
              burn_init = 10000,
              iter_rec = 20000,
    ...
    for chain in 1:n_chains
        ...
        current_log_post = log_posterior(model, θ_current, log_jac_current, prior_info, y, X, a1, P1;
        ...
        for s in 1:iter_init
            ...
            log_post_star = log_posterior(model, θ_star, log_jac_star, prior_info, y, X, a1, P1;
                                          filter_type=filter_type, N_particles=N_particles, rng=rng)
        ...
        for s in 1:iter_rec
            ...
            if s > burn_rec
                if filter_type == "kalman"
                    _, α_samp, _ = diffuse_kalman_filter_alternative(
                        model, y, X, θ_current, a1, P1; do_smooth=true, do_sim_smooth=true, rng=rng)
                        α_draws[s, :, :] = α_samp
                # elseif filter_type == "particle"
                #     _, α_samp = ParticleFilter.particle_filter(
                #         model, y, θ_current, a1, P1; N_particles=N_particles)
                end 
            end
...
#########################
function run_chain(chain::Int, model, y, X, prior_info, a1, P1;
                   filter_type="kalman",
                   iter_init=20000,
                   burn_init=10000,
                   iter_rec=20000,
    ...
    for s in 1:iter_rec
        ...
        if s > burn_rec
            ...
            # elseif filter_type == "particle"
            #     _, α_samp = ParticleFilter.particle_filter(
            #         model, y, θ_current, a1, P1; N_particles=1000)
            end
            α_draws[s, :, :] = α_samp
        end
...
#########################
function MCMC_estimation_parallel(model, y, X, prior_info_collection::Vector, a1, P1;
    filter_type="kalman",
    iter_init = 20000,
    burn_init = 10000,
    iter_rec = 20000,
    ...
    Threads.@threads for chain in 1:n_chains
        ...
        res = run_chain(chain, model, y, X, prior_info_collection[chain], a1, P1;
