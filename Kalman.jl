...
export diffuse_kalman_filter, kalman_filter, kalman_smoother, diffuse_kalman_filter_alternative
...
#########################
#  Diffuse Kalmam Filter (from Durbin & Koopman 2000 Fast Filtering and Smoothing for Multivariate State Space Models)
#########################
function diffuse_kalman_filter(model, y, X, θ, α1, P1; do_smooth = true, do_sim_smooth = false, F_tol = 1e-8, rng=Random.GLOBAL_RNG)
...
function diffuse_kalman_filter_alternative(model, y, X, θ, α1, P1; do_smooth = true, do_sim_smooth = false, F_tol = 1e-8, rng=Random.GLOBAL_RNG)
    # Get state-space matrices.
    # Now assume get_state_space returns:
    # Z, H, d, T, R, Q, c, D, P_diffuse
    Z, H, d, T, R, Q, c, P_diffuse, D = get_state_space(model, θ)
    P1 = copy(P1)
    ...
    for t in 1:m
        for i in 1:n
            # When measurement is missing (e.g., second variable) skip the update:
            if i == 2 && t in missing_period
                v = 0  # Skip update for the second variable
                F = 1e-9
                F_diff = 0.0  # Define F_diff to avoid UndefVarError
                ...
                # Compute the innovation including the exogenous contribution.
                # The predicted measurement is: d + D*X[:, t] + Z*α.
                # Note: X is now used in the measurement equation.
                v = (y[i, t] - d[i] - dot(D[i, :], X[:, t]) - dot(Z[i, :], α))[1]
                F = Z[i, :]' * P * Z[i, :] + H[i, i]
                F_diff = Z[i, :]' * P_diff * Z[i, :]
                if F > F_tol || F_diff > F_tol
                    K = P * Z[i, :]
        ...
        # --- State Prediction ---
        if t < m
            # No exogenous input in the state update now:
            α = T * α 
            P = T * P * T' + R * Q * R'
            P_diff = T * P_diff * T'
            α_f[:, t+1] = α
